...
# Main title
st.title('Dashboard - Option Pricing using Various Methods')

# User selected model from sidebar 
pricing_method = st.sidebar.radio('Select option method for option pricing', options=[model.value for model in OPTION_PRICING_MODEL])
...
if pricing_method == OPTION_PRICING_MODEL.BLACK_SCHOLES.value:
    ...
    ticker = st.text_input('Ticker symbol', 'GOOG')
    strike_price = st.number_input('Strike price', 250)
    ...
    sigma = st.slider('Sigma (%)', 0, 100, 20)
    exercise_date = st.date_input('Exercise date', min_value=datetime.today() + timedelta(days=1), value=datetime.today() + timedelta(days=365))
    number_of_simulations = st.slider('Number of simulations', 50, 100000, 10000)
    ...
    sigma = st.slider('Sigma (%)', 0, 100, 20)
    exercise_date = st.date_input('Exercise date', min_value=datetime.today() + timedelta(days=1), value=datetime.today() + timedelta(days=365))
    number_of_time_steps = st.slider('Number of time steps', 1000, 100000, 15000)
